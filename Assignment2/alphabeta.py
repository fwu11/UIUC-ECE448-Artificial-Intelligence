...
class ALPHABETA():
            ...
            for i in range(4):
                ...
                for j in range(5):
                    ...
                    if not (True in player): 
                        if sum(opponent) == 5 or sum(opponent)==4:        
    ...
    def getBestMove(self, board, side):
        ...
        bestMove = self.getMaxValue(board, 3 - 1, side,alpha,beta)
    ...
    def getMaxValue(self, board, depth, side,alpha,beta):
        # player side
        ...
        for idx in range(len(empty_space)):
            ...
            if len(empty_space) == 1 or depth == 0:
                ...
                result = self.getMinValue(board, depth - 1, side,alpha,beta)
    ...
    def getMinValue(self, board, depth, side,alpha,beta):
        ...
        for idx in range(len(empty_space)):
            ...
            if len(empty_space) == 1 or depth == 0:
                ...
                result = self.getMaxValue(board, depth - 1, side,alpha,beta)
                node.score = result.score
            if node.score <= alpha:
                board[node.y][node.x] = 0
                return node
